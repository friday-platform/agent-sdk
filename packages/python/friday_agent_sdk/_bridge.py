...
async def _validate_async(validate_id: str) -> None:
    """Publish agent metadata for registration handshake then exit."""
    ...
    if reg.display_name is not None:
        metadata["displayName"] = reg.display_name
    if reg.llm is not None:
        metadata["llm"] = reg.llm
    ...
    try:
        await nc.publish(f"agents.validate.{validate_id}", json.dumps(metadata).encode())
    finally:
        await nc.drain()
